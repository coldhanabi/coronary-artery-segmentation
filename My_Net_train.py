...
from nets.Mynet3_7 import mynet
from utils.training import get_lr_scheduler, set_optimizer_lr, weights_init
from utils.callbacks import LossHistory
from utils.data_loader import MyDataset, My_dataset_collate
from utils.utils import download_weights
from utils.utils_fit import fit_one_epoch
# from divide import divide_data
...
if __name__ == "__main__":
    ...
    model = mynet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
    ...
    with open(os.path.join(Data_path, "Dataset/ImageSets/Segmentation/train.txt"), "r") as f:
        train_lines = f.readlines()
    with open(os.path.join(Data_path, "Dataset/ImageSets/Segmentation/val.txt"), "r") as f:
    ...
    if True:
        ...
        gen = DataLoader(train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                         pin_memory=True,
                         drop_last=True, collate_fn=My_dataset_collate, sampler=train_sampler)
        gen_val = DataLoader(val_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                             pin_memory=True,
                             drop_last=True, collate_fn=My_dataset_collate, sampler=val_sampler)
